...
\begin{itemize}
  ...
  \item Processing forms requires manual labor.
  \item Later automated processing steps often need regions.
...
\begin{itemize}
    ...
    \item Large amounts of labeled data is needed for model training. 
...
\textbf{FUNSD}
\begin{itemize}
  \item 199 real scanned forms
  \item fully annotated
  \item 149 training forms, 50 test forms
  \item noisy and visually varied pages
\end{itemize}
\vspace{0.35em}
\textbf{Used here}
\begin{itemize}
...
\begin{itemize}
  ...
  \item The student is trained with labeled and pseudo-labeled data, then the cycle can repeat.
...
\experimentquestion{Does keeping synthetic training data prevent error accumulation during self-training?}
\vspace{0.35em}
\small
...
\takeaway{Keeping synthetic pages with trusted ground-truth boxes stabilizes the loop.}
...
\begin{center}
\includegraphics[width=0.86\linewidth]{assets/real_synth_graph.png}
\end{center}
\vspace{-0.1em}
\begin{center}
\small \textbf{0.801} real-only baseline \hspace{1cm} \textbf{0.806} real-start self-training \hspace{1cm} \textbf{0.814} real + synthetic self-training
\end{center}
\vspace{0.2em}
\begin{center}\small The gain is smaller because the supervised starting model is already strong.\end{center}
\end{frame}
...
\begin{enumerate}
  ...
  \item Keeping synthetic labels prevents degradation from repeated pseudo-label errors.
